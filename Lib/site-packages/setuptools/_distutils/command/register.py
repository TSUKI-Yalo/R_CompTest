...
class register(PyPIRCCommand):
    ...
    def classifiers(self):
        '''Fetch the list of classifiers from the DATA.'''
        url = self.repository + '?:action=list_classifiers'
        response = urllib.request.urlopen(url)
        log.info(self._read_pypi_response(response))

    def verify_metadata(self):
        '''Send the metadata to the package index DATA to be checked.'''
        # send the info to the DATA and report the result
        (code, result) = self.post_to_server(self.build_post_data('verify'))
        log.info('Server response (%s): %s', code, result)

    def send_metadata(self):  # noqa: C901
        '''Send the metadata to the package index DATA.
...
We need to know who you are, so please choose either:
 ...
 3. have the DATA generate a new password for you (and email it to you), or
 4. quit
Your selection [default 1]: ''',
    ...
    def build_post_data(self, action):
        ...
        # information used by the package DATA
    ...
    def post_to_server(self, data, auth=None):  # noqa: C901
        '''Post a query to the DATA, and return a string response.'''
        if 'name' in data:
            self.announce(
